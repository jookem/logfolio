...
const strategyRiskReturn = useMemo(() => {
  const byStrategy = {};
  plList.forEach(tr => {
    if (!tr.strategy) return;
    if (!byStrategy[tr.strategy]) byStrategy[tr.strategy] = [];
    byStrategy[tr.strategy].push(tr.pl);
  });
  return Object.entries(byStrategy)
    .filter(([, pls]) => pls.length >= 2)
    .map(([strategy, pls]) => {
      const n = pls.length;
      const mean = pls.reduce((s, v) => s + v, 0) / n;
      const variance = pls.reduce((s, v) => s + (v - mean) ** 2, 0) / n;
      const stdDev = Math.sqrt(variance) || 0.01;
      const sharpe = mean / stdDev;
      return { strategy, mean, stdDev, sharpe, count: n };
    });
}, [plList]);
