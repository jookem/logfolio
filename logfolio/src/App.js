    ...
    try {
      ...
      rows.forEach((row) => {
        ...
        const rawPrice =
          ...
          row.price ||
          "0";
        const rawShares = row.filled || row["total qty"] || row.shares || "0";
        const price =
          parseFloat(String(rawPrice).replace(/[^0-9.-]/g, "")) || 0;
        const shares =
          parseFloat(String(rawShares).replace(/[^0-9.-]/g, "")) || 0;
        const date = parseDate(
          row["filled time"] || row["placed time"] || row.date || ""
        );
      ...
      const trades = [];
      Object.entries(byTicker).forEach(([ticker, orders]) => {
        const buys = orders.filter((o) => o.hasBuy && !o.hasSell);
        const sells = orders.filter((o) => o.hasSell && !o.hasBuy);
        const both = orders.filter((o) => o.hasBuy && o.hasSell);
        ...
        both.forEach((o) => {
          trades.push({
            ...
            type: "stock",
            direction: "long",
            entryPrice: parseFloat(o.raw.entryprice),
            exitPrice: parseFloat(o.raw.exitprice),
            shares: o.shares,
            strategy: "Breakout",
            emotion: "Calm",
            mistake: "None",
      ...
      if (trades.length === 0) {
        setError("No completed trades found. Only closed trades will import.");
        return;
      }
      ...
      >
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <textarea
            ...
            placeholder="Or paste CSV/TSV content here..."
          />
        </div>
        {error && (
          <div
        ...
        {preview.length > 0 && (
          <div style={{ marginBottom: 14 }}>
            ...
            >
              {preview.slice(0, 4).map((tr, i) => (
                <div
                  ...
                  style={{
                    ...
                    padding: "9px 12px",
                    borderBottom:
                      i < preview.length - 1 ? `1px solid ${t.border}` : "none",
                  }}
                >
                  <span
                    style={{
              ...
              {preview.length > 4 && (
                <div
                  style={{ padding: "7px 12px", fontSize: 12, color: t.text3 }}
                >
